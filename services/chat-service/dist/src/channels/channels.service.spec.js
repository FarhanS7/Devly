...
describe('ChannelsService', () => {
    ...
    beforeEach(async () => {
        ...
        service = module.get(channels_service_1.ChannelsService);
        prisma = module.get(prisma_service_1.PrismaService);
        teamsService = module.get(teams_service_1.TeamsService);
    });
    afterEach(() => {
        jest.clearAllMocks();
    });
    ...
    describe('create', () => {
        ...
        it('should create a private channel and add only the creator', async () => {
            const privateDto = { ...createDto, type: client_1.ChannelType.PRIVATE };
            const channel = {
                id: 'channel-1',
                teamId,
        ...
        it('should throw BadRequestException if channel name already exists', async () => {
            ...
            await expect(service.create(teamId, userId, createDto)).rejects.toThrow(common_1.BadRequestException);
        });
        it('should throw if user is not admin or owner', async () => {
            mockTeamsService.verifyAdminOrOwner.mockRejectedValue(new common_1.ForbiddenException('Not authorized'));
            await expect(service.create(teamId, userId, createDto)).rejects.toThrow(common_1.ForbiddenException);
        });
    });
    describe('findTeamChannels', () => {
        it('should return all public and user\'s private channels', async () => {
            ...
            const userId = 'user-1';
            const channels = [
                { id: 'ch-1', name: 'General', type: client_1.ChannelType.PUBLIC },
                { id: 'ch-2', name: 'Private', type: client_1.ChannelType.PRIVATE },
            ];
    ...
    describe('findOne', () => {
        ...
        it('should throw NotFoundException if channel does not exist', async () => {
            mockPrismaService.channel.findUnique.mockResolvedValue(null);
            await expect(service.findOne('non-existent', 'user-1')).rejects.toThrow(common_1.NotFoundException);
        });
    });
    describe('update', () => {
        ...
        it('should throw NotFoundException if channel does not exist', async () => {
            mockPrismaService.channel.findUnique.mockResolvedValue(null);
            await expect(service.update('non-existent', 'user-1', {})).rejects.toThrow(common_1.NotFoundException);
        });
    });
    describe('remove', () => {
    ...
    describe('addMember', () => {
        ...
        it('should throw BadRequestException if user already a member', async () => {
            ...
            await expect(service.addMember(channelId, userId, addMemberDto)).rejects.toThrow(common_1.BadRequestException);
        });
    });
    ...
    describe('verifyMembership', () => {
        ...
        it('should throw ForbiddenException if user is not a member', async () => {
            mockPrismaService.channelMember.findUnique.mockResolvedValue(null);
            await expect(service.verifyMembership('channel-1', 'user-1')).rejects.toThrow(common_1.ForbiddenException);
        });
    });
    describe('isMember', () => {
...
//# sourceMappingURL=channels.service.spec.js.map